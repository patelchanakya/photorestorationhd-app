-- Fix increment_back_to_life_usage to call check_and_increment_usage with correct number of parameters
-- The atomic function now expects 8 parameters but we were calling it with 6
...
BEGIN
  ...
  -- Call the atomic function with Pro user settings - NOW WITH CORRECT PARAMETERS
  SELECT check_and_increment_usage(
    ...
    v_transaction_id,            -- p_store_transaction_id TEXT
    v_transaction_id             -- p_original_transaction_id TEXT (same for store: format)
  ) INTO v_success;
...
COMMENT ON FUNCTION increment_back_to_life_usage IS 'Handles video generation limits for Pro users. Supports orig:, store:, and fallback: user ID formats with detailed logging. Fixed to call check_and_increment_usage with correct 8 parameters.';