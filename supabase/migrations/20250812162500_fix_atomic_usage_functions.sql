-- Migration: Fix atomic usage functions date comparison
-- Fix the data type issue with date comparisons

-- Function to atomically check and increment usage (FIXED VERSION)
...
-- Function to rollback usage increment (FIXED VERSION)
...
BEGIN
  ...
  WHERE 
    ...
    AND last_video_date = today_date::TEXT;
