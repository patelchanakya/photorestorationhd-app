-- Migration: Remove daily limits for monthly plans
-- Weekly plans keep 1 video per day limit, monthly plans have no daily restrictions

-- Function to atomically check and increment usage (REMOVE DAILY LIMITS FOR MONTHLY)
...
DECLARE
  ...
  needs_reset BOOLEAN;
BEGIN
  -- Get today's date as DATE type (to match column type)
  ...
  SELECT 
    ...
    store_transaction_id,
    CASE 
      -- Daily limits only apply to weekly plans
      WHEN p_plan_type = 'weekly' AND (last_video_date IS NOT NULL AND last_video_date = today_date) THEN false
      ELSE true
    END
  ...
  IF NOT can_use_today THEN
    RETURN false; -- Daily limit reached (weekly plans only)
  END IF;
  ...
  WHERE 
    ...
    AND (
      -- For weekly plans: check daily limit
      p_plan_type != 'weekly' OR last_video_date IS NULL OR last_video_date != today_date
    );
