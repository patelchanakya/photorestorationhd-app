-- Migration: Fix date comparison type mismatch in atomic function
-- Fixes: operator does not exist: date <> text

-- Function to atomically check and increment usage (FIXED DATE COMPARISON)
...
DECLARE
  ...
  needs_reset BOOLEAN;
BEGIN
  -- Get today's date as TEXT to match column type
