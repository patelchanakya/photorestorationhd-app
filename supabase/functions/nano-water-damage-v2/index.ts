...
interface NanoWaterDamageRequest {
...
serve(async (req) => {
  ...
  try {
    // Get request body
    const { image_data, custom_prompt, user_id: requestUserId }: NanoWaterDamageRequest = await req.json()
    ...
    console.log('💧 Starting nano-water-damage generation with webhooks...')

    // Determine the prompt to use - water damage specific
    ...
    // Build input for nano-banana generation (water damage mode)
    const input = {
      prompt,
    ...
    const { error: insertError } = await supabase
      ...
      .upsert({
        ...
        mode: 'water_damage', // Same mode as original
        status: 'starting',
        style_key: null,
        input: {
          prompt: prompt,
    ...
    console.error('❌ Nano-water-damage generation error:', error)
    ...
    if (user_id && user_id !== 'anonymous') {
      try {
        ...
        console.log('✅ Photo usage rollback completed after nano-water-damage generation error');
      } catch (rollbackError) {
    ...
    return new Response(
      JSON.stringify({ 
        error: 'Failed to start nano-water-damage generation',
