...
serve(async (req) => {
  ...
  try {
    // Get request body
    const { image_data, custom_prompt, user_id: requestUserId }: NanoRepairRequest = await req.json()
    ...
    console.log('🔧 Starting nano-repair generation with webhooks...')

    // Determine the prompt to use - repair specific
    ...
    // Build input for nano-banana generation (repair mode)
    const input = {
      prompt,
    ...
    const { error: insertError } = await supabase
      ...
      .upsert({
        ...
        mode: 'repair', // Same mode as original
        status: 'starting',
        style_key: null,
        input: {
          prompt: prompt,
    ...
    console.error('❌ Nano-repair generation error:', error)
    ...
    if (user_id && user_id !== 'anonymous') {
      try {
        ...
        console.log('✅ Photo usage rollback completed after nano-repair generation error');
      } catch (rollbackError) {
    ...
    return new Response(
      JSON.stringify({ 
        error: 'Failed to start nano-repair generation',
