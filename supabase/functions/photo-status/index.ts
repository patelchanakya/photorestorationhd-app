...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📊 Status for ${prediction_id}: ${prediction.status} (${progress}%) - ${prediction.mode} mode`)

    // Build response with mode-specific information
    const response: any = {
      success: true,
    ...
    if (prediction.input) {
      // Add prompt information for all modes
      ...
      switch (prediction.mode) {
        ...
        case 'background':
          ...
          break
        case 'enhance':
          response.enhance_mode = prediction.input.enhance_mode || prediction.style_key
          response.has_custom_prompt = prediction.input.has_custom_prompt
          break
        case 'custom':
...
/* 
This unified status endpoint handles status checking for all photo modes:
- repair
- outfit (with style information)
- background (with style information) 
- custom (with custom prompt)
- enhance (with enhance mode: unblur, colorize, descratch, enlighten)

To invoke:
curl -i --location --request POST 'http://localhost:54321/functions/v1/photo-status' \
  --header 'Authorization: Bearer [anon_key]' \
  --header 'Content-Type: application/json' \
  --data '{"prediction_id":"gm3qorzdhgbfurvjtvhg6dckhu"}'

Response example for outfit mode:
{
  "success": true,
  ...
  "is_successful": true,
  "has_output": true,
  "style_title": "Job Interview",
  "prompt_used": "Replace the subject's clothing with smart business casual attire...",
