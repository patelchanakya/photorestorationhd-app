...
serve(async (req) => {
  ...
  try {
    console.log('🪝 Photo repair webhook received')
    ...
    console.log(`📝 Updating prediction ${webhookData.id} with status: ${webhookData.status}`)
    ...
    console.log(`✅ Successfully updated prediction ${webhookData.id}`)

    // Log successful completion
    if (webhookData.status === 'succeeded' && outputUrl) {
      console.log(`🎉 Photo repair completed successfully: ${outputUrl}`)
    } else if (webhookData.status === 'failed') {
      console.log(`❌ Photo repair failed: ${webhookData.error || 'Unknown error'}`)
    }
